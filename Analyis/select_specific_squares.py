...
def pick_linear(n=100):
    """Select n squares by the linear distribution of activity"""

    # Assuming df is your DataFrame with columns sid and activity
    # and PKL_FILE is the path to your pickle file
    df = pd.read_pickle(PKL_FILE, compression=None)
...
def plot_selection(selection):
    ...
    plt.title('Sum of CDRs for Squares selected linearly', fontsize=22)
    plt.xticks([])
    plt.show()
...
if __name__ == "__main__":
    ...
    plot_selection(median_selections)
    linear_selections = pick_linear()
    plot_selection(linear_selections)
